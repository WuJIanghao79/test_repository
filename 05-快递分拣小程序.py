...
for item in old_data:
    if item[1][0:3] not in position:
        position.append(item[1][0:3])

# 将各个地名赋值为key
for i in range(len(position)):
    new_data[position[i]] = []

# 依次遍历地区，如果地区匹配到了一个快递信息，将该快递信息追加到对应的字典中
for i in range(len(position)):
    print(position[i])
    for j in range(len(old_data)):
        if position[i] in old_data[j][1]:
            new_data[position[i]].append(old_data[j])
